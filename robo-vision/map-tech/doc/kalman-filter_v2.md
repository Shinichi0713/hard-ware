...
真値に対して、誤差のある計測値に対して、推定値が誤差を抑えて真値に近似する値を推定している。

![1760325581679](image/kalman-filter_v2/1760325581679.png)

## 分散共分散行列

カルマンフィルタや統計学・機械学習を理解するうえで、

**「分散共分散行列（covariance matrix）」** は非常に重要な概念です。

ひとことで言うと：

> **分散共分散行列とは、複数の変数の「ばらつき」と「相関の強さ」をまとめて表す行列** です。

---

## 🧮 1. まずは1変数の場合：分散（variance）

1つの変数 ( x ) の分散は、

「平均からどれくらい散らばっているか（ばらつき）」を表す量です。
...
平均 (5) のまわりにデータが ±2 程度ばらつくなら、分散はおよそ (2^2 = 4)。

---

## 🔢 2. 2変数以上になると：共分散（covariance）

2つの変数 (x, y) の関係（同時に増減する傾向）を表すのが **共分散** です。
...
* 正の値 → (x) が大きくなると (y) も大きくなる（正の相関）
* 負の値 → (x) が大きくなると (y) は小さくなる（負の相関）
* 0 → 無相関（関係がない）

---

## 🧩 3. 複数変数をまとめると「分散共分散行列」
...
| 非対角成分 | 変数どうしの共分散（相関の強さ・方向） |

---

## 📊 5. 図で直感的に理解する

2次元データ ((x_1, x_2)) の散布図を考えると、

* 分散が大きいほど → 広くばらついた楕円になる
* 共分散が正 → 右上がりの楕円（正の傾き）
* 共分散が負 → 右下がりの楕円（負の傾き）

この楕円の「形」と「向き」を決めているのが **分散共分散行列 (\Sigma)** です。
...
状態の「不確かさ（どの程度あいまいか）」を **共分散行列 (P)** で表しています。

* (P) が大きい → 状態推定が不確か（広い範囲に分布）
* (P) が小さい → 状態推定が確か（狭い範囲に集中）

そして、カルマンフィルタの各ステップで

[

P_{t|t-1},\quad P_{t|t}

]

を更新することで、「予測の信頼性」が時間とともに変化します。
...
| ---------------------- | ------------------------------------------- |
| 対称行列               | (\Sigma^\top = \Sigma)                      |
| 正定値                 | (a^\top \Sigma a > 0)（ばらつきは常に非負） |
| 線形変換に対して       | (\mathrm{Cov}(A x) = A \Sigma A^\top)       |
| ガウス分布の形を決める | 平均と共分散で確率密度の楕円を決定できる    |
...
> **分散共分散行列**とは、
>
> 多次元のデータ（ベクトル）がどの方向にどれだけばらついているか、
>
> そして変数どうしがどれくらい一緒に動くかを表す行列です。

カルマンフィルタではこの行列を「状態の信頼性」として扱い、

観測のたびに「より確からしい方向へ」分散を絞っていきます。

---

もし希望があれば、

📈「2次元ガウス分布の楕円と共分散の違いを図で可視化」

を Python で描いてお見せできます。やってみましょうか？

# イメージ

![1760344423453](image/kalman-filter_v2/1760344423453.png)


[世界一分かりやすいカルマンフィルタの理論と導出と実装 | 機械学習と情報技術](https://disassemble-channel.com/the-theory-of-kalman-filter/)
