...
* **並列共振回路（タンク回路）** は、インダクタ LL とコンデンサ CC を並列に接続したものです。
* 交流入力に対して、回路全体のインピーダンスは

Z(jω)=(1jωL+jωC)−1Z(j\omega) = \left( \frac{1}{j\omega L} + j\omega C \right)^{-1}
となります。
...
インピーダンスの大きさが極端に変化する周波数は

ω0=1LC⇒f0=12πLC\omega_0 = \frac{1}{\sqrt{LC}} \quad \Rightarrow \quad f_0 = \frac{1}{2\pi \sqrt{LC}}
です。
...
* **並列共振の場合**
  * インダクタとコンデンサを流れる電流が逆位相になり、ちょうど打ち消し合います。
  * 外部から見ると、回路にほとんど電流が流れないため、**インピーダンスは理想的には無限大（最大値）** になります。
  * → **電流が最小・電圧が最大** になる動作。
* **直列共振の場合**
  * L と C が逆位相で電圧を打ち消し合うため、全体のインピーダンスが最小（理想的には 0Ω）になります。
  * →  **電流が最大・インピーダンスが最小** 。
...
* コイルやコンデンサには損失（抵抗成分）があるため、インピーダンスが「無限大」になることはありません。
* その代わり、インピーダンスが周波数特性の中で「最も大きくなる」ピークが現れます。

---

✅ 結論:

「 **LC並列共振した場合、インピーダンスは最大値となる** 」という表現は正しいです。ただし、理想素子では無限大、実際の回路では有限の大きな値になります。

---

ご希望なら、「直列共振と並列共振の違いを図付きで整理」してみましょうか？
